...
> 状态：**W-1–W-5 已落地**（工作面简化主线完成；可选后置：默认进待办 / 待办行密度）  
...
| 数据 | — | `GET /api/dashboard`：计数 + awaiting/in_review；无未展示列表字段 |
...
| B-1 | 缺陷操作入口去重 | ✅ |
| D-2 | 默认入口二选一 | 未做（可后置） |
| D-3 | 去掉未展示的 `active_tasks` / `failed_tasks` / `failed_count` | ✅ **W-5** |
| D-4 | 不做 Usage / 多看板 | — |
...
| --- | --- | --- |
| B-1 | 去重操作入口 | ✅ **W-1** |
| B-2 | 缺陷行 live chip | ✅ **W-4** |
| B-3 | 工作项执行记录折叠 | ✅ **W-4** |
| B-4 | 去掉未渲染标签变量；标题 tooltip 带标签；清理无用 badge CSS | ✅ **W-5** |
| B-5 | 不做 Board / Gantt | — |
...
| 日期 | 说明 |
| --- | --- |
| 2026-09-03 | **W-5 落地**：dashboard 去掉未用 `active_tasks`/`failed_tasks`/`failed_count`；缺陷表去掉死标签变量与无用 badge CSS，标题 tooltip 带标签 |
| 2026-09-03 | **W-4 落地**：缺陷行 running/awaiting live chip；工作项执行记录活跃置顶 + 历史折叠 |
