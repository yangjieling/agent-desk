...
> 状态：**W-1 · W-2 已落地**；下一迭代 **W-3**（待办文案 + Reject 弹层）  
...
| 交互 | 未读/归档、筛选、行内 live agent、详情复用 Issue | 选择闸门 chips / 验收 Accept·Reject；Reject 用 `prompt` |
...
| # | 项 | 做法 | 优先级 |
| --- | --- | --- | --- |
| I-1 | **待办成为唯一「人介入」主面** | 文案统一「待办」；总览只摘抄 | 高 · **W-3** |
| I-2 | **Reject 体验** | 用小弹层/内联备注替代 `window.prompt` | 中 · **W-3** |
| I-3 | **空态区分** | 真无待办引导 | 低 |
| I-4 | **可选：行密度** | 卡片改紧凑行 | 低 |
